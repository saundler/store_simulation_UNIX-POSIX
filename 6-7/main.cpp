...
int print_descriptor;                        // Дескриптор семафора вывода сообщений
std::vector<int> needed_descriptor(3);       // Дескрипторы семафоров для вызова продавца
std::vector<int> available_descriptor(3);    // Дескрипторы семафоров обозначающих что продавец свободен
...
void exit_func(int sig) {
    if (sig == SIGINT) {
        if (main_pid == getpid()) {
            ...
            close(print_descriptor);          // Закрытие дескриптора семафора для вывода сообщений
            shm_unlink(print_name.c_str());   // Удаление семафора для вывода сообщений 

            // Закрытие семафоров для ожидания и вызова продавцов
            for (int i = 0; i < 3; ++i) {
                std::string sem_name = needed_base_name + std::to_string(i);
                close(needed_descriptor[i]);     // Закрытие дескриптора семафора для вызова продавца
                shm_unlink(sem_name.c_str());    // Удаление семафора для вызова продавца

                sem_name = available_base_name + std::to_string(i);
                close(available_descriptor[i]);  // Закрытие дескриптора семафора для ожидание продавца
                shm_unlink(sem_name.c_str());    // Удаление семафора для ожидание продавца
            }
        }
        exit(0);
    }
...
int main(int argn, char *argv[]) {
    ...
    print_descriptor = shm_open(print_name.c_str(), O_CREAT | O_EXCL | O_RDWR, 0666);
    ftruncate(print_descriptor, sizeof(sem_t));
    print_mutex = (sem_t *) mmap(0, sizeof(sem_t), PROT_WRITE | PROT_READ, MAP_SHARED, print_descriptor, 0);
    sem_init(print_mutex, 1, 1);

    for (int i = 0; i < 3; ++i) {
        std::string sem_name = needed_base_name + std::to_string(i);
        needed_descriptor[i] = shm_open(sem_name.c_str(), O_CREAT | O_EXCL | O_RDWR, 0666);
        ftruncate(needed_descriptor[i], sizeof(sem_t));
        needed[i] = (sem_t *) mmap(0, sizeof(sem_t), PROT_WRITE | PROT_READ, MAP_SHARED, needed_descriptor[i], 0);
        sem_init(needed[i], 1, 0);

        sem_name = available_base_name + std::to_string(i);
        available_descriptor[i] = shm_open(sem_name.c_str(), O_CREAT | O_EXCL | O_RDWR, 0666);
        ftruncate(available_descriptor[i], sizeof(sem_t));
        available[i] = (sem_t *) mmap(0, sizeof(sem_t), PROT_WRITE | PROT_READ, MAP_SHARED, available_descriptor[i], 0);
        sem_init(available[i], 1, 1);
    }
